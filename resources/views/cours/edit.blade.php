                        ...
                        <div class="flex items-center justify-between gap-3 flex-wrap">
                            <div>
                                <div class="text-xs text-white/60">
                                    ...
                                    <a href="{{ route('cours.index', (int)$coursRow->matiere_id) }}" class="hover:text-white">Cours</a>
                                    <span class="mx-1">/</span>
                                    <span class="text-white font-semibold">Modifier</span>
                                </div>
                                <h1 class="mt-1 text-xl font-bold text-white">Modifier le cours</h1>
                                <p class="mt-1 text-sm text-white/70">Matière : {{ $matiereRow->nom ?? '' }}</p>
                            </div>
                            <a href="{{ route('cours.index', (int)$coursRow->matiere_id) }}" class="btn btn-ghost">← Retour</a>
                        </div>
                    </div>

                    <div class="p-5">
                        <form method="POST" action="{{ route('cours.update', $coursRow->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="classe_id" class="glass-input w-full rounded-xl px-3 py-2 text-sm" required>
                                        @foreach(($classes ?? collect()) as $cl)
                                            <option value="{{ $cl->id }}" {{ (int)old('classe_id', $coursRow->classe_id) === (int)$cl->id ? 'selected' : '' }}>
                                                {{ $cl->nom }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <label class="inline-flex items-center gap-2 mt-2 text-white/80">
                                        <input type="checkbox" name="actif" value="1" {{ old('actif', (int)($coursRow->actif ?? 1)) ? 'checked' : '' }}>
                                        <span>Actif</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <input name="titre" value="{{ old('titre', $coursRow->titre) }}" class="glass-input w-full rounded-xl px-3 py-2 text-sm" required>
                            </div>

                            <div class="mt-4">
                                <label class="block mb-1">Contenu</label>
                                <textarea name="contenu" rows="8" class="glass-input w-full rounded-xl px-3 py-2 text-sm">{{ old('contenu', $coursRow->contenu) }}</textarea>
                            </div>

                            <div class="mt-5 flex items-center gap-2 flex-wrap">
                                <button class="btn btn-primary" type="submit">Mettre à jour</button>
                                <a class="btn btn-ghost" href="{{ route('cours.index', (int)$coursRow->matiere_id) }}">Annuler</a>
                            </div>
                        </form>
                    </div>
                </div>
